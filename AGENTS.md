...
## Build Entrypoint
- `./build-termux.sh` is the primary CLI; default run deletes modified files, use `-d`/`--dirty` to preserve artifacts
- New flags: `--skip-download` (skip git clone), `--skip-bootstrap-build` (use existing artifacts)
...
- Smart patching: skips patches when target directory doesn't exist (disabled apps)
...
## Dev Branch Features
- **Parallel downloads**: git clones run in background with `&` and `wait`
- **Parallel CI builds**: 3-job workflow (bootstraps matrix, addons, main app)
- **CI uses Azure mirrors**: `sed -i 's/archive.ubuntu.com/azure.archive.ubuntu.com/g'` for reliability

## CI Notes
- Workflow: `.github/workflows/build-termux.yml` (runs on `ubuntu-24.04`, JDK 21)
- Also triggers on `dev` branch pushes (not just main)
- Mandatory: Fixes all `.sh` line endings with `dos2unix` to prevent Windows clone issues
- Bootstraps built in parallel matrix (aarch64, arm, x86_64, i686), artifacts passed to main app job
- Cancels duplicate runs for the same ref; uploads APKs and build logs as artifacts
